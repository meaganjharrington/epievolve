...
# rodent <img src="man/figures/logo.png" align="right" height="139"/>
...
rodent relies on the odin2-dust2-monty ecosystem from RESIDE at Imperial
College London. Install these first from r-universe:
...
# View Rt estimates (median and 95% credible intervals)
head(out_flu$Rt_quantiles)

# View beta estimates
head(out_flu$beta_t_quantiles)

# View susceptible trajectory
head(out_flu$S_quantiles)

# Plot Rt
library(ggplot2)

ggplot(out_flu$Rt_quantiles, aes(x = time)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.3) +
  geom_line(aes(y = median)) +
  geom_hline(yintercept = 1, linetype = "dashed", colour = "red") +
  labs(x = "Time (days)", y = "Rt",
       title = "Estimated Rt — Influenza H1N1 2009, Pennsylvania")
```
...
For full methodology and validation results, see the accompanying thesis
report.
